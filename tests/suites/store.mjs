...
export async function run({ base }) {
  ...
  */
  ...
  // Both of these describe the branch the project just left. `poll_sha` kept
  // across a switch makes the next poller tick compare the new branch's head
  // against a SHA from the old one, find them different, and deploy unasked —
  // the baseline guard does not fire because it tests for null.
  ...
  r.check(
    "changing branch releases a pin belonging to the old branch",
    afterBranch.body.pinned_sha === null,
    String(afterBranch.body.pinned_sha)
  );
