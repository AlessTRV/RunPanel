  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-foreground-400">Connect your GitHub account to browse repositories</p>
      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              <Icon icon="solar:link-bold-duotone" width={24} className="text-purple-400" />
              <div>
            ...
            {hasToken ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-foreground-300">GitHub connected</span>
                </div>
        ...
        {hasToken && (
          <div>
            ...
            {loadingRepos ? (
              ...
              <p className="text-sm text-foreground-500 text-center py-8">No repositories found</p>
            ) : (
              <div className="space-y-2">
                {filtered.map((repo) => (
                  <div key={repo.id} className="flex items-center justify-between rounded-xl bg-white/[0.04] px-4 py-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold truncate">{repo.name}</p>
                        {repo.private && <span className="text-[10px] text-foreground-500 bg-white/[0.06] px-1.5 py-0.5 rounded">private</span>}
                        {repo.language && <span className="text-[10px] text-foreground-500">{repo.language}</span>}
                      </div>
                      {repo.description && <p className="text-xs text-foreground-500 truncate mt-0.5">{repo.description}</p>}
                    </div>
                    <p className="text-[11px] text-foreground-500 flex-shrink-0 ml-3">{repo.default_branch}</p>
                  </div>
                ))}
              </div>
            )}
